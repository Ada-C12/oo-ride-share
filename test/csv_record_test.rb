...
describe RideShare::CsvRecord do
  describe 'constructor' do
    ...
    it 'validates the ID' do
      id = -7
      expect {RideShare::CsvRecord.new(id)}.must_raise ArgumentError
    end
  end
  
  describe 'load_all' do
    it "raises an error if neither full_path nor directory is provided" do
      expect {RideShare::CsvRecord.load_all}.must_raise ArgumentError
    end
    
    it "raises an error if invoked directly (without subclassing)" do
      full_path = 'test/test_data/testrecords.csv'
      expect {RideShare::CsvRecord.load_all(full_path: full_path)}.must_raise NotImplementedError
    end
  end
  ...
  describe 'validate_id' do
    ...
    it 'raises for negative numbers and 0' do
      [0, -1, -10, -9999].each do |id|
        expect {RideShare::CsvRecord.validate_id(id)}.must_raise ArgumentError
      end
    end
    
    it 'raises for nil' do
      expect {RideShare::CsvRecord.validate_id(nil)}.must_raise ArgumentError
    end
  end
  ...
  describe 'extension' do
    ...
    describe 'load_all' do
      let(:record_count) {%x{wc -l 'test/test_data/testrecords.csv'}.split(' ').first.to_i - 1}
